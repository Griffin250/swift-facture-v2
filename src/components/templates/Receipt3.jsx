      ...
      >
        <div className="flex-grow px-2">
          <header className="mb-3 text-center border-b-2 border-dashed pb-2">
            <div className="text-xl font-extrabold tracking-widest text-slate-700">{t('receiptTemplate.receipt')}</div>
          ...
          <section className="mb-2 flex justify-between text-xs">
            <div className="text-slate-600">{t('receiptTemplate.customer')}</div>
            <div className="font-medium text-slate-800">{billTo || t('receiptTemplate.na')}</div>
          </section>

          <section className="mb-2 flex justify-between text-xs">
            <div className="text-slate-600">{t('receiptTemplate.cashier')}</div>
          ...
          <section className="py-2 mb-2">
            <div className="grid grid-cols-4 font-bold text-xs mb-2">
              ...
              <span className="text-center">{t('receiptTemplate.qty')}</span>
              <span className="text-center">Amt</span>
              <span className="text-right">{t('receiptTemplate.total')}</span>
            </div>
            {items.map((item, index) => (
              <div key={index} className="grid grid-cols-4 items-center mb-1 text-xs">
                <span className="truncate">{`${index + 1}. ${item.name || t('receiptTemplate.na')}`}</span>
          ...
          <footer className="mt-2 text-xs">
            ...
            <div className="flex justify-between py-2 border-t border-dashed border-slate-400 mt-2">
              <div className="text-base font-semibold">{`${items.length} ${t('receiptTemplate.item')}s`}</div>
